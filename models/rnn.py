    ...
    def forward(self, inputs, lengths):
        ...
        if self.config.bidirectional:
            ...
            if self.config.cell == 'gru':
                ...
                #state[0]=h_n, state[1]=c_n
                #[::2]是以step為2取資料，只取出正向序列的資料
...
class rnn_decoder(nn.Module):
    ...
    def forward(self, input, state, output=None, mask=None): 
        ...
        #output = [batch, hidden_size],state=([3,batch, hidden_size],[3,batch, hidden_size])
...
class StackedLSTM(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        for i, layer in enumerate(self.layers):
            ...
            h_1 += [h_1_i] #把當前這層的結果與上一層相加(是表示串接下去(高機率)，還是真的把數值相加?)
            c_1 += [c_1_i]
